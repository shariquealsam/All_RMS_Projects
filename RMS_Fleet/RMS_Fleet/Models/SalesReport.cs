    ...
    public class SalesReportData
    {
        ...
        public DataSet GetDocumnetTypeReport(string DocumentType)
        {
            ...
            try
            {
                ...
                sSql = " SELECT BRNCH.BranchName, FVD.VehicleNo AS VehicleNumber, FMTD.Make, Model, ChessisNumber, ExpiryDate, ReceivingDate, " +
                        ...
                        " inner JOIN (select * from [dbo].[Fleet_Maintenance_Tax_Details]WITH(NOLOCK) where Document in (" + DocumentType + ") ) FMTD " +
                        " ON FMTD.VehicleNumber=FVD.VehicleNo " +
                        " LEFT JOIN Fleet_Branch_Details BRNCH WITH(NOLOCK) " +
                        " ON FVD.BranchId=BRNCH.BranchId  " +
