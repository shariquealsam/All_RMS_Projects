    ...
    public class UserController : Controller
    {
        ...
        public JsonResult GetUserDetails()
        {
            ...
            for (int i = 0; i <= dt.Rows.Count - 1; i++)
            {
                ...
                //update
                U.RecId = dt.Rows[i]["Rec_Id"].ToString();
                //-----------------------------------------------------
        ...
        [HttpPost ]
        public JsonResult UpdateUserStatus(string RecId, string status)
        {
            var resultSuccess = new { Message = "Updated Successfully." };
            var resultError = new { Message = "Error Please Check." };

            int return1 = 0; 

            return1  = UD.UpdateUserDetail(RecId,status );
