    ...
    public class KpiController : Controller
    {
        ...
        public JsonResult GetMasterDetails(string VechileNumber)
        {
            ...
            catch (Exception ex)
            {
                ...
                ci.SendMailMessage("support@sisprosegur.com", "sharique.aslam@sisprosegur.com", "sharique.aslam@sisprosegur.com", "", "Error Occurred: - GetMasterDetails", ex.Message.ToString() + "|" + ex.StackTrace.ToString());
        ...
        public JsonResult GetBranch(string RegionId)
        {
            ...
            catch (Exception ex)
            {
                ...
                ci.SendMailMessage("support@sisprosegur.com", "sharique.aslam@sisprosegur.com", "sharique.aslam@sisprosegur.com", "", "Error Occurred: - GetBranch", ex.Message.ToString() + "|" + ex.StackTrace.ToString());
        ...
        public JsonResult SaveOpeningClosingDetails(string RegionId, int BranchID, string VechileNumber, double OpeningKM, double Openingfuel)
        {
            ...
            catch (Exception ex)
            {
                ...
                //ci.SendMailMessage("support@sisprosegur.com", "sharique.aslam@sisprosegur.com", "sharique.aslam@sisprosegur.com", "", "Error Occurred: - SaveOpeningClosingDetails", ex.Message.ToString() + "|" + ex.StackTrace.ToString());
                Message = ex.Message;
            }
            return Json(Message, JsonRequestBehavior.AllowGet);
        }
        ...
        [HttpPost]
        public ActionResult DownloadPendency(FormCollection Fd)
        {
            Get_from_config();
            ...
            try
            {
                if (UserType == "Admin")
                {
                    query1 = "SELECT [RegionName],[BranchName],Total, Performned FROM ( " +
                            "SELECT COUNT([VehicleNo]) As Total,COUNT(VechileNumber) As Performned,[RegionId],[BranchId] " +
                            " FROM " +
                            " ( " +
                            ...
                            " LEFT OUTER JOIN " +
                            " ( " +
                            " SELECT DISTINCT VechileNumber " +
                            " FROM [dbo].[Fleet_KPL_Branch_Details] A WITH(NOLOCK) " +
                            " WHERE FORMAT([Month],'MMMM-yyyy')=FORMAT(DATEADD(MONTH,-1,GETDATE()),'MMMM-yyyy') " +
                            " )tblKpl ON TblMaster.[VehicleNo] = tblKpl.VechileNumber " +
                            " Group BY [RegionId],[BranchId] ) tblFinal" +
                            " LEFT JOIN " +
                            " ( " +
                ...
                using (XLWorkbook wb = new XLWorkbook())
                {
                    ...
                    Response.Charset = "";
                    Response.ContentType = "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet";
                    Response.AddHeader("content-disposition", "attachment;filename=KPLPendencyReport.xlsx");
                    using (MemoryStream MyMemoryStream = new MemoryStream())
                    {
                        wb.SaveAs(MyMemoryStream);
                        MyMemoryStream.WriteTo(Response.OutputStream);
