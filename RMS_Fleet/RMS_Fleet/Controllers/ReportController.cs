...
namespace RMS_Fleet.Controllers
{
    public class ReportController : Controller
    {
        ...
        public ActionResult NoActivityVehicles(string RegionId,string Type)
        {
            try
            {
                if (Session["RegionIdForNoActivityVehicles"] != null || Session["RegionIdForNoActivityVehicles"].ToString() != RegionId || Session["Type"] != null || Session["Type"]!=Type)
                {
                    Session["RegionIdForNoActivityVehicles"] = RegionId;
                    Session["Type"] = Type;
                }
                Session["RegionIdForNoActivityVehicles"].ToString();
            ...
            bool isNumberic = CheckNumeric(Session["RegionIdForNoActivityVehicles"].ToString());
            if (isNumberic == true && Type!="")
            {
                ds = R.GetNoActivityVehicles(Session["RegionIdForNoActivityVehicles"].ToString(), Session["Type"].ToString());

                if (ds.Tables.Count > 0 && ds.Tables[0].Rows.Count > 0)
                {
                    List<Report> lstNoActivityVehicle = new List<Report>();
                    for (int i = 0; i < ds.Tables[0].Rows.Count; i++)
                    {
                        Report rt = new Report();

                        rt.RegionId = Convert.ToInt32(ds.Tables[0].Rows[i]["RegionId"].ToString());
                        rt.RegionName = ds.Tables[0].Rows[i]["RegionName"].ToString();
                        rt.BranchId = Convert.ToInt32(ds.Tables[0].Rows[i]["BranchId"].ToString());
                        rt.BranchName = ds.Tables[0].Rows[i]["BranchName"].ToString();
                        rt.VechileNumber = ds.Tables[0].Rows[i]["VehicleNo"].ToString();

                        lstNoActivityVehicle.Add(rt);
                    }

                    ViewBag.lstNoActivityVehicle = lstNoActivityVehicle;
                }
            }
        ...
        public ActionResult NoActivityVehiclesDownload(string RegionId,string Type)
        {
            try
            {
                if (Session["UserName"].ToString() == "" || Session["UserName"] == null)
            ...
            try
            {
                ...
                bool isNumberic = CheckNumeric(RegionId);
                if (isNumberic == true && Type != "")
                {
                    ds = R.GetNoActivityVehicles(RegionId, Type);
                    ...
                    if (ds.Tables.Count > 0 && ds.Tables[0].Rows.Count > 0)
                    {
                        ...
                        using (XLWorkbook wb = new XLWorkbook())
                        {
                            ...
                            Response.AddHeader("content-disposition", "attachment;filename=NoActivityVehicles.xlsx");
            ...
            return RedirectToAction("NoActivityVehicles");
        }
        ...
        public JsonResult ReportMaster(string FromDate, string ToDate, string RegId, string Branchid, string IsFilter, string IsSessionRegionID)
        {
            ...
            for (int i = 0; i <= dt.Rows.Count - 1; i++)
            {
                ...
                H.Route_Id = dt.Rows[i]["Route_Id"].ToString();
                ...
                {
                    H.OpeningDate = Convert.ToDateTime(dt.Rows[i]["OpenDate"].ToString()).ToString("dd-MMM-yyyy");
                ...
                {
                    H.ClosingDate = Convert.ToDateTime(dt.Rows[i]["CloseDate"].ToString()).ToString("dd-MMM-yyyy");
        ...
        [HttpGet]
        public ActionResult ReportMasterInExcel()
        {
            ...
            string IsFilter=Request.QueryString["IsFilter"].ToString();
            string Date = DateTime.Now.ToString("yyyy-MM-dd");
            string RegionId = "0";
            string BranchIds = "0";
           ...
           string IsSessionRegionID = Session["RegionIds"].ToString();
            DataSet ds = new DataSet();
        ...
        public JsonResult SaveRevisedEntries(string RevisedOpeningKM, string RevisedClosingKM, int OCKey)
        {
            ...
            if (RevisedOpeningKM == null)
            {
                RevisedOpeningKM = "";
            }

            if (RevisedClosingKM == null)
            {
                RevisedClosingKM = "";
            }
